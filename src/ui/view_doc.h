// view_doc.h — the document view: caret, selection, word wrap and drawing.
//
// This is the editor. It owns the caret and the drag-selection state, wraps lines
// to the viewport, hit-tests a click back to a byte offset, keeps the syntax
// highlighter's per-line cookies, and draws the result.
//
// It shows a pf::ui::text_buffer. It does not know where that buffer came from, so
// choosing a highlighter for it is the caller's job — see set_buffer.
...
namespace pf::ui
{
	...
	public:
		doc_view(view_host& host, const theme& th, view_context* context = nullptr) : text_view(host, th, context),
		                               _highlight(syntax::for_language(syntax::language::plain))
		{
		}

		~doc_view() override = default;

		// Whether this view accepts edits. The application uses it to decide which
		// menu a right-click deserves.
		[[nodiscard]] virtual bool is_editable() const { return false; }

		// The buffer on show, and where a click lands in it. Both are public so an
		// application can build a context menu about the text under the cursor.
		[[nodiscard]] const text_buffer_ptr& buffer() const { return _doc; }
		[[nodiscard]] text_location text_at(const pf::ipoint& client_pt) const { return client_to_text(client_pt); }
		...
		// Show a buffer, and say how to highlight it. The view cannot choose a
		// highlighter itself: that depends on where the text came from, which is
		// exactly what this layer does not know.
		virtual void set_buffer(const text_buffer_ptr& d, highlight_fn highlight)
		{
			_scroll_offset = {};
			_doc = d;
			_highlight = highlight ? std::move(highlight)
			                       : syntax::for_language(syntax::language::plain);
			reset_parse_cookies();
		                        ...
		                        const std::span<text_block> buf, int& nBlocks) const
		{
			...
			if (_doc->spell_check() && !buf.empty())
			{
				...
				auto append_block = [](std::vector<text_block>& blocks, const int pos, const text_style color)
				{
					...
					if (pos == 0 && color == text_style::normal_text)
						return;
				...
				auto append_segment = [&](const int start, const int end, const text_style base_color)
				{
					...
					while (pos < end)
					{
						...
						if (spell::is_word_byte(line_text[pos]))
						{
							while (next < end && spell::is_word_byte(line_text[next]))
								next++;

							if (!spell::check_word(line_text.substr(pos, next - pos)))
								color = text_style::error_text;
						}
						else
						{
							while (next < end && !spell::is_word_byte(line_text[next]))
				...
				if (nBlocks > 0)
				{
					append_segment(0, buf[0]._char_pos, text_style::normal_text);
				...
				{
					append_segment(0, len, text_style::normal_text);
				}
		                 ...
		                 pf::measure_context& measure) override
		{
			...
			_hscroll.set_dpi_scale(_theme.dpi_scale);

			// A width change invalidates every break point
			mark_wrap_dirty_all();
			layout();

			// The host's scrollbar signal names the document pane, so a view in its
			// own window would never see its own scrollbar brought up to date
			if (_doc)
				recalc_vert_scrollbar();

			_host.invalidate_scrollbar(), _host.invalidate_layout();
		}
	...
	public:
		void recalc_vert_scrollbar() override
		{
			...
			if (_scroll_offset.y > max_y)
			{
				...
				_host.invalidate_status(), _host.invalidate_caret();
			}
	...
	protected:
		...
		uint32_t on_mouse_leave()
		{
			...
			if (need_redraw) _host.invalidate_status();
			return 0;
		}
	...
	protected:
		...
		[[nodiscard]] pf::color_t line_bg_color(const int lineIndex) const
		{
			...
			return _theme.style_color(text_style::normal_bkgnd);
		}
			...
			if (nCount > 0)
			{
				...
				if (nSelBegin < nSelEnd)
				{
					draw_line(draw, ptOrigin, rcClip, pszChars, nOffset + nSelBegin, nSelEnd - nSelBegin,
					          f, _theme.style_color(text_style::sel_text), _theme.style_color(text_style::sel_bkgnd));
				}
				if (nSelEnd < nCount)
				{
		                       ...
		                       const pf::font& f) const
		{
			...
			for (int row = 0; row < num_rows; row++)
			{
				...
				auto draw_clipped = [&](const int seg_start, const int seg_end, const text_style color)
				{
					...
					if (cs < ce)
					{
						draw_line(draw, origin, row_rc, color, line_view, cs, ce - cs,
						          text_location(cs, lineIndex), f, _theme.style_color(color), bg_color);
					}
				};

				if (nBlocks > 0)
				{
					draw_clipped(0, pBuf[0]._char_pos, text_style::normal_text);
					for (int i = 0; i < nBlocks; i++)
					{
				...
				{
					draw_clipped(0, nLength, text_style::normal_text);
				}
				...
				if (row == num_rows - 1 && _doc->is_inside_selection(text_location(nLength, lineIndex)))
				{
					...
					draw.fill_solid_rect(sel_x, row_rc.top, _font_extent.cx, row_rc.height(),
					                     _theme.style_color(text_style::sel_bkgnd));
				}
			}
		}
		...
		void draw_line(pf::draw_context& draw, const pf::irect& rc, const int lineIndex, const pf::font& f) const
		{
			...
			if (nBlocks > 0)
			{
				...
				draw_line(draw, origin, rc, text_style::normal_text, line_view, 0, pBuf[0]._char_pos,
				          text_location(0, lineIndex),
				          f, _theme.style_color(text_style::normal_text), bg_color);
				...
				for (auto i = 0; i < nBlocks - 1; i++)
				{
					...
					draw_line(draw, origin, rc, pBuf[i]._color, line_view,
					          ...
					          f, _theme.style_color(pBuf[i]._color), bg_color);
				}
				...
				draw_line(draw, origin, rc, pBuf[nBlocks - 1]._color, line_view,
				          ...
				          f, _theme.style_color(pBuf[nBlocks - 1]._color), bg_color);
			}
			else
			{
				draw_line(draw, origin, rc, text_style::normal_text, line_view, 0, nLength,
				          text_location(0, lineIndex),
				          f, _theme.style_color(text_style::normal_text), bg_color);
			}
		...
		void draw_view(pf::window_frame_ptr& window, pf::draw_context& draw) const override
		{
			...
			draw.fill_solid_rect(rcClient, _theme.style_color(text_style::normal_bkgnd));

			// Paint margin background
			const pf::irect rcMargin(0, 0, margin_w, rcClient.bottom);
			draw.fill_solid_rect(rcMargin, _theme.style_color(_sel_margin ? text_style::sel_margin : text_style::normal_bkgnd));
		...
		void draw_drop_marker(pf::draw_context& draw) const
		{
			...
			if (pt.y + _font_extent.cy > rc.top && pt.y < rc.bottom)
				draw.fill_solid_rect(pt.x, pt.y, 2, _font_extent.cy, _theme.style_color(text_style::md_bullet));
		}
		...
		virtual void draw_caret(pf::draw_context& draw) const
		{
			...
			if (caret_rect.top < rc.bottom && caret_rect.bottom > rc.top)
				draw.fill_solid_rect(caret_rect, _theme.text);
		}
	};
}
